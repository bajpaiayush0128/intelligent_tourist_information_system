from flask import Flask, render_template,request,json,redirect,url_for,session, Blueprint
import requests
import json
import sqlite3
import pandas as pd
...
def tourismRecommendationSystem():
    ...
    if request.method == "POST":
        ...
        return render_template('index1.html',states=states,labels=labels,weathers=weathers,recreations=recreations,similar_spots = spots,visible="")
    return render_template('index1.html',states=states,labels=labels,weathers=weathers,recreations=recreations,visible="invisible")
